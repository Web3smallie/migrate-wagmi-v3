...
export function transformStepInvoke(source: string): TransformResult {
  let changed = false;
  let code = source;
  ...
  code = code.replace(pattern, (match, inner) => {
    ...
    const newInner = inner.replace(
      /function\s*:\s*["'][^"']*["']/,
      `function: referenceFunction({ appId: "${appId}", functionId: "${functionId}" })`
    );
