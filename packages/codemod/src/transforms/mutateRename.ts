...
export function transformMutateRename(source: string): TransformResult {
  let changed = false;
  let code = source;
  ...
  for (const { hook, oldFn, oldAsync } of mutationHooks) {
    ...
    const asyncPattern = new RegExp(
      `(const\\s*\\{[^}]*?)\\b${oldAsync}\\b([^}]*?\\}\\s*=\\s*${hook}\\s*\\()`,
      "g"
    );
