...
export function transformEventSchemas(source: string): TransformResult {
  let changed = false;
  let code = source;
  ...
  if (code.includes("EventSchemas")) {
    ...
    code = code.replace(
      /new\s+EventSchemas\s*\(\s*\)/g,
      `/* TODO (AI): EventSchemas removed in v4. Migrate to eventType() or staticSchema().
    ...
    // Flag the import too
    code = code.replace(
      /import\s*\{([^}]*EventSchemas[^}]*)\}\s*from\s*["']inngest["']/g,
      (match) => {
