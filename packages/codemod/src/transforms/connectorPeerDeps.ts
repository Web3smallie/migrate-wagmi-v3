...
export function transformConnectorPeerDeps(source: string): TransformResult {
  let changed = false;
  let code = source;
  ...
  for (const [connector, dep] of Object.entries(connectorDeps)) {
    if (dep === "none required") continue;

    // Check if this connector is imported from wagmi/connectors
    const importPattern = new RegExp(
      `\\b${connector}\\b.*from\\s*["']wagmi/connectors["']|from\\s*["']@wagmi/connectors["'].*\\b${connector}\\b`,
      "g"
    );

    if (
      code.includes(connector) &&
