...
exports.transformConnectorImports = transformConnectorImports;
function transformConnectorImports(source) {
    let changed = false;
    let code = source;
   ...
   See: https://wagmi.sh/react/guides/migrate-from-v2-to-v3 */\n`;
    for (const pkg of connectorPackages) {
        if (code.includes(pkg) && !code.includes("TODO (AI): wagmi v3 moved connectors")) {
            changed = true;
            const importPattern = new RegExp(`import\\s*\\{[^}]*\\}\\s*from\\s*["']${pkg.replace("/", "\\/")}["']`, "g");
...
//# sourceMappingURL=connectorImports.js.map