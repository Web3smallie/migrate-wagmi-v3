...
exports.transformCreateConfig = transformCreateConfig;
function transformCreateConfig(source) {
    let changed = false;
    let code = source;
    // Flag autoConnect removal
    if (code.includes("autoConnect")) {
        changed = true;
        code = code.replace(/autoConnect\s*:\s*(true|false)\s*,?/g, `/* TODO (AI): autoConnect was removed in wagmi v3. 
   ...
   See: https://wagmi.sh/react/guides/migrate-from-v2-to-v3 */`);
    ...
    if (code.includes("publicClient")) {
        changed = true;
        code = code.replace(/publicClient\s*:/g, `/* TODO (AI): publicClient was removed in wagmi v3. 
   ...
   transports:`);
    ...
    if (code.includes("webSocketPublicClient")) {
        changed = true;
        code = code.replace(/webSocketPublicClient\s*:/g, `/* TODO (AI): webSocketPublicClient was removed in wagmi v3.
