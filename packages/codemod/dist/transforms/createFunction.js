...
exports.transformCreateFunction = transformCreateFunction;
function transformCreateFunction(source) {
    let changed = false;
    let code = source;
    ...
    code = code.replace(variablePattern, (match, fn, opts, triggerVar, asyncPart) => {
        if (opts.includes("triggers"))
            return match;
        if (triggerVar === "undefined" || triggerVar === "null")
            return match;
        changed = true;
        const optsInner = opts.slice(1, -1).trimEnd().replace(/,\s*$/, "");
        const newOpts = `{\n      ${optsInner.trim()},\n      triggers: [${triggerVar}],\n    }`;
