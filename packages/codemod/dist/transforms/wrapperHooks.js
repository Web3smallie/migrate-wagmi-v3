...
exports.transformWrapperHooks = transformWrapperHooks;
function transformWrapperHooks(source) {
    let changed = false;
    let code = source;
    ...
    for (const hook of deprecatedHooks) {
        ...
        const wrapperPattern = new RegExp(`((?:export\\s+)?(?:const|function)\\s+\\w+\\s*=?\\s*(?:\\([^)]*\\)\\s*=>|function\\s*\\([^)]*\\))\\s*\\{[^}]*\\b${hook}\\b[^}]*\\})`, "gs");
...
//# sourceMappingURL=wrapperHooks.js.map