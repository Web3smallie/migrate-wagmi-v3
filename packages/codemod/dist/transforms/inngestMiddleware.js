...
exports.transformInngestMiddleware = transformInngestMiddleware;
function transformInngestMiddleware(source) {
    let changed = false;
    let code = source;
    ...
    if (code.includes("middleware") && code.includes("inngest")) {
        ...
        if (oldMiddlewarePattern.test(code)) {
            ...
            code = code.replace(oldMiddlewarePattern, (match) => {
                if (match.includes("@codemod-ai-action"))
                    return match;
...
${match}`;
        ...
        if (code.includes("createMiddleware")) {
            changed = true;
            code = code.replace(/\bcreateMiddleware\b/g, `/* TODO (AI): createMiddleware was replaced by middleware() in Inngest v4 */ middleware`);
        }
    }
    return { code, changed };
}
